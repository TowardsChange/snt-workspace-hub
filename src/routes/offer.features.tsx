...
const summary = [
  { label: "Charge totale", value: "198", unit: "j-h", sub: "≈ 10 mois-personne" },
  { label: "Calendrier (équipe de 4)", value: "≈ 11", unit: "sem.", sub: "marge incluse pour revue et corrections" },
  { label: "Première démo", value: "≈ 4", unit: "sem.", sub: "Phase 0 + Phase 1" },
...
const phases: Phase[] = [
  {
    ...
    rows: [
      { name: "Authentification Microsoft (MSAL)", desc: "Connexion Azure AD / Entra ID via MSAL, gestion de session, déconnexion, rafraîchissement de jeton. Indispensable pour SPIE ; comble la lacune MFA de Batisia.", size: "M", effort: 6 },
      { name: "Ossature applicative et navigation", desc: "Barre latérale à sections groupées (Application / Offre), barre supérieure, mise en page responsive, sélecteur de langue (FR / EN), cloche de notifications, fil d'Ariane.", size: "M", effort: 5 },
      { name: "Rôles et permissions", desc: "Trois rôles (Admin, Manager d'équipe, Utilisateur), ACL par projet et par document, drapeau « Tous » au niveau de l'espace de travail, fenêtre de partage.", size: "L", effort: 6 },
      { name: "Gestion des utilisateurs et des équipes", desc: "Flux d'invitation, objets équipe, liste des membres, accès documentaire par équipe, transfert de propriété.", size: "M", effort: 4 },
      { name: "Paramètres et profil", desc: "Profil utilisateur, paramètres de l'espace de travail, préférences de notifications, langue, thème.", size: "S", effort: 2 },
      { name: "Journal d'audit et observabilité", desc: "Qui a demandé quoi, quelle réponse a été produite, quelle source a été citée. Indispensable pour les usages sensibles côté achats. Lacune de Batisia.", size: "M", effort: 5 },
    ],
  },
  {
    tag: "PHASE 1",
    ...
    rows: [
      { name: "Pipeline RAG", desc: "Ingestion documentaire, découpage adapté au BTP (gestion des structures CCTP, CCAP, DCE), embeddings, base vectorielle. Cible de latence d'indexation < 1 min/doc — contre ≈ 10 min chez Batisia.", size: "XL", effort: 18 },
      { name: "Orchestration des LLM", desc: "Abstraction multi-fournisseurs (Mistral, GPT, Claude, modèle interne), réponses en streaming, gestion des templates de prompt, suivi des coûts. Compatible BYO-LLM.", size: "M", effort: 7 },
      { name: "Base documentaire", desc: "Arborescence hiérarchique (Mémoires Technique → projets), liste de fichiers avec pastilles de statut (indexation / prêt / échec), zone de dépôt, import par lots, OCR de secours, actions par document.", size: "M", effort: 10 },
      { name: "Composeur de conversation unifié", desc: "Champ de saisie « Que cherchez-vous ? » avec trombone → modale de choix des sources, déclencheur @ → sélecteur d'agents, bouton d'envoi, streaming des tokens, pastilles de citations en ligne.", size: "L", effort: 10 },
      { name: "Modale de choix des sources", desc: "Modale à 3 onglets (Depuis l'ordinateur / Base documentaire / Bibliothèques de prix), arborescence avec déploiement et +, sélection multiple, pastille de recherche globale.", size: "S", effort: 5 },
      { name: "Visionneuse documentaire et citations", desc: "Visionneuse PDF latérale, vignettes de pages avec cadre orange sur la page active, zoom, navigation par page, passage cité surligné en jaune, citations multi-passages avec score de confiance.", size: "M", effort: 7 },
      { name: "Historique", desc: "Panneau des conversations passées, recherche, reprise de n'importe quelle conversation, renommage, archivage, partage avec un collègue.", size: "S", effort: 4 },
    ],
  },
  {
    tag: "PHASE 2",
    ...
    days: 71,
    weeks: "≈ 3,5 semaines calendaires",
    ...
    rows: [
      { name: "Bibliothèques de prix — Recherche guidée", desc: "Filtres structurés (nature de travaux, rubrique BPU, plage de dates, géographie), résultats tabulaires, export. Indexation du corpus historique de prix.", size: "M", effort: 8 },
      { name: "Bibliothèques de prix — Recherche IA", desc: "Recherche en langage naturel réutilisant le composeur de conversation, périmètre limité au corpus de prix, avec citations renvoyant à l'opération d'origine.", size: "S", effort: 4 },
      { name: "Comparer des devis", desc: "Import des propositions fournisseurs hétérogènes, normalisation des lignes, tableau comparatif côte à côte, recommandation IA, export Excel. Promesse marketing : 15 min contre 3 heures.", size: "XL", effort: 20 },
      { name: "Conformité", desc: "Vérification d'un document contre un référentiel contractuel ou réglementaire ; rapport d'écarts avec sévérité, citation et correction suggérée. Référentiels BTP préchargés + personnalisés.", size: "XL", effort: 18 },
      { name: "Rédaction assistée", desc: "Rédaction de mémoires techniques, synthèses et sections appuyées sur le corpus de l'entreprise. Plateau de citations en direct, édition / affinage, régénération par section, export DOCX/PDF.", size: "L", effort: 15 },
      { name: "Réponses inter-corpus", desc: "Réponse unique combinant une clause de la Base documentaire et un prix des Bibliothèques de prix — une fonctionnalité que Batisia laisse entendre mais ne livre pas visiblement.", size: "M", effort: 6 },
    ],
  },
  {
    tag: "PHASE 3",
    ...
    rows: [
      { name: "Agents — exécution", desc: "Sélecteur déclenché par @, invocation d'agents sur les sources attachées, exécution séquencée de plusieurs prompts, rendu structuré des réponses. Livré avec les 10 agents d'analyse de risques SPIE préconfigurés.", size: "M", effort: 8 },
      { name: "Agents — éditeur", desc: "Éditeur visuel pour créer, versionner, partager et paramétrer des agents. Bac à sable de test, enregistrement en brouillon, publication dans l'espace de travail.", size: "M", effort: 8 },
      { name: "Intégrations natives", desc: "SharePoint, OneDrive, Teams, ainsi que des connecteurs vers la GED et l'ERP de SPIE. Aucune intégration nommée sur le site de Batisia — barrière concurrentielle.", size: "M", effort: 7 },
      { name: "Annotation documentaire", desc: "Annotations en ligne et commentaires partagés sur les pages de la visionneuse PDF. Transforme la visionneuse en lecture seule de Batisia en surface de travail.", size: "M", effort: 6 },
      { name: "Mobile et tablette", desc: "Passe responsive pour les ingénieurs sur chantier consultant les documents sur tablette. Batisia n'a aujourd'hui aucune présence mobile.", size: "S", effort: 4 },
      { name: "Finition, performance, supervision", desc: "Passe de performance sur la recherche et la visionneuse, gestion des erreurs, Sentry/Grafana, analytics, accessibilité (WCAG), recette finale.", size: "S", effort: 5 },
    ],
  },
];
...
const recapStats = [
  { label: "Phase 0", value: "28" },
  { label: "Phase 1", value: "61" },
  { label: "Phase 2", value: "71" },
  { label: "Phase 3", value: "38" },
];
...
const notes: React.ReactNode[] = [
  ...
  "Phase 0 + Phase 1 = la première démo fonctionnelle présentée au conseil (≈ 4 semaines, 89 j-h). Les phases 2–3 ajoutent la parité concurrentielle et les extensions.",
